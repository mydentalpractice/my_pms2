...
def apply_voucher():
    ...
    treatmentid = int(common.getkeyvalue(request.vars,"treatmentid","0"))
    treatment = common.getkeyvalue(request.vars,"treatment","")
    memberid = int(common.getkeyvalue(request.vars,"memberid","0"))
    patient = common.getkeyvalue(request.vars,"patient","")
    fullname = common.getkeyvalue(request.vars,"fullname","")
    plan = common.getkeyvalue(request.vars,"plan","RPIP599")

    r = db(db.patientmember.id == memberid).select(db.patientmember.city,db.patientmember.st,db.patientmember.company)
    city = r[0].city if(len(r)>0) else "Jaipur"
    st = r[0].st if(len(r)>0) else "Rajasthan (RJ)"
    ...
    if(rspobj["result"] == "fail"):
        ...
        vlist = rspobj["VOUCHER_DATA"]
        
        
    formA = SQLFORM.factory(
            Field('voucher','string'),
            Field('treatmentid','integer',default=treatmentid),
            Field('memberid','integer',default=memberid),
            Field('cityid','integer',default=cityid),
            )
    ...
    returnurl = URL('payment', 'list_payment', vars=dict(page=1, providerid=providerid))
    return dict(formA=formA,returnurl=returnurl,vlist=vlist, providerid=providerid,providername=providername,treatment=treatment,treatmentid=treatmentid,
                memberid=memberid,patient=patient,fullname=fullname,city=city,state=st,plan=plan)
...
def create_payment():
    ...
    voucher_url = URL('payment','apply_voucher',vars=dict(page=page,providerid=providerid,memberid=memberid,patient=patient,\
                                                          fullname=fullname,treatment=treatment,treatmentid=treatmentid,plan=policy))
    ...
    return dict(formA=formA,formB=formB,formC=formC, patient=patient,fullname=fullname,hmopatientmember=hmopatientmember,providername=provdict["providername"],
                ...
                treatmentcost=paytm["treatmentcost"],copay=paytm["copay"],inspays=paytm["inspays"],companypays=paytm["companypays"],walletamount=paytm["walletamount"],
                totaltreatmentcost=paytm["totaltreatmentcost"],totalinspays=paytm["totalinspays"],totalcopay=paytm["totalcopay"],
                                ...
                                totalwalletamount=paytm["totalwalletamount"],policy=policy,
...
def update_payment():
    ...
    if(returnurl == ""):
        returnurl = URL('payment','list_payment',vars=dict(page=page,providerid=providerid,treatmentid=treatmentid))
